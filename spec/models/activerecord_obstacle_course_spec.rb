...
describe 'ActiveRecord Obstacle Course' do
  ...
  it "13. groups an order's items by name" do
    ...
    # ------------------ Using ActiveRecord ----------------------
    grouped_items = Item.where(id: [expected_result])
    # ------------------------------------------------------------
  ...
  it '15. gets all item names associated with all orders' do
    ...
    # ----------------------- Using Ruby -------------------------
    # names_1 = Order.all.map do |order|
    #   if order.items
    #     order.items.map { |item| item.name }
    #   end
    # end
    #
    # names = names_1.flatten
    # ------------------------------------------------------------
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    users = User.select(:name).distinct.joins(:order_items)
        .where("order_items.item_id=#{item_8.id}").order(:name).pluck(:name)
    # ------------------------------------------------------------
  ...
  it '22. calculates the total sales for all but one user' do
    ...
    # ------------------ Using ActiveRecord ---------------------
    total_sales = User.joins(:orders).where('user_id !=?', 2).sum(:amount)
    # -----------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data =  Order.joins(:order_items, :user).group(:order_id)
      .select("users.name AS user_name, order_id, count(item_id) AS item_count").order("name DESC")
    # ---------------------------------------------------------------
